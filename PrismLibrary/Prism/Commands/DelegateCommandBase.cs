...
namespace Prism.Commands
{
    ...
    public abstract class DelegateCommandBase : ICommand, IActiveAware
    {
        ...
        private int _executionState = 0;
        public bool AllowReentrance { get; set; }
        ...
        {
            if (AllowReentrance == false && Interlocked.Exchange(ref _executionState, 1) == 1)
            {
                return;
            }
            _isExecuting = true;
            await _executeMethod(parameter).ContinueWith(task =>
            {
        ...
        {
            if (AllowReentrance == false &&_isExecuting) return false;
