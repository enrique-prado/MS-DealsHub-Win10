...
namespace Prism.Tests.Mvvm
{
    ...
    public class DelegateCommandFixture
    {
        ...
        [TestMethod]
        public async Task NonGenericDelegateCommandShouldInvokeExplicitExecuteFunc()
        {
            bool executed = false;
        ...
        #region BlockReentrance

        [TestMethod]
        async public Task DelegateCommandDoesNotAllowReentrance()
        {
            var executions = 0;
            ...
            var secondExecutionCompletionSource = new TaskCompletionSource<bool>();
            var queueDoubleExecution = Task.Delay(250).ContinueWith(async task =>
            {
                try
                {
                    Assert.AreEqual(1, executions, "1st execution should be in flight");
                    Assert.IsFalse(command.CanExecute(), "command.CanExecute() should be false at time of 2nd execution");
                    await command.Execute();
                    secondExecutionCompletionSource.SetResult(true);
                }
                catch(Exception)
                {
                    secondExecutionCompletionSource.SetResult(false);
                }
            });            
            await command.Execute();
            var verified = await secondExecutionCompletionSource.Task;
            Assert.AreEqual(1, executions, "executions 1");
            Assert.IsTrue(verified, "verified");
        }

        [TestMethod]
        async public Task DelegateCommandAllowReentranceWorks()
        {
            var executions = 0;
            var command = DelegateCommand.FromAsyncHandler(async () =>
            {
            ...
            var queueDoubleExecution = Task.Delay(250).ContinueWith(async task =>
            {
                ...
                {
                    Assert.AreEqual(1, executions, "1st execution should be in flight");
                    Assert.IsTrue(command.CanExecute(), "command.CanExecute() should be true at time of 2nd execution");
                    await command.Execute();
                    secondExecutionCompletionSource.SetResult(true);
                }
                catch (Exception)
            ...
            await command.Execute();
            var verified = await secondExecutionCompletionSource.Task;
            Assert.AreEqual(2, executions, "executions");
            Assert.IsTrue(verified, "verified");
        }
